...
export default function DeveloperSignatory() {
  ...
  useEffect(() => {
    const fetchIdTypes = async () => {
      try {
        ...
        setIdTypes(Array.isArray(types) ? types : []);
      } catch (error) {
  ...
  const validateForm = (formData) => {
    ...
    if (!formData.get("valid_id_type_id2")) {
      newErrors.valid_id_type_id2 =
        "Second valid ID type is required.";
    }

    const idImage2 = formData.get("image2");

    if (!(idImage2 instanceof File) || idImage2.size === 0) {
      newErrors.image2 = "Second valid ID image is required.";
    }

    if (!formData.get("number2")?.trim()) {
      newErrors.number2 =
        "Second valid ID number is required.";
    }

    if (!formData.get("expiration_date2")) {
      newErrors.expiration_date2 =
        "Second valid ID expiration date is required.";
    }
  ...
  const submitSignatory = async (event) => {
    ...
    if (sameAsPresent) {
      addressFields.forEach((field) => {
        const presentValue =
          form.elements[`present_${field}`]?.value || "";
    ...
    // IMPORTANT:
    // Do not set loading if validation fails.
    ...
    try {
      ...
      data.append(
        ...
        formData.get("valid_id_type_id2") || ""
      );
      ...
      alert(
        error?.response?.data?.message ||
          ...
          error?.message ||
          "Failed to submit signatory details."
      );
    } finally {
      // ALWAYS stop spinner after API request
      setLoading(false);
    }
  };
          ...
          >
            ...
            <div className="form-row">
              ...
              <div className="input-field">
                ...
                <button
                  type="button"
                  className="file-label upload-button"
                  onClick={() =>
            ...
            <div className="form-row">
              ...
              <div className="input-field">
                ...
                <ErrorMessage field="expiration_date2" />
